...
public class WorkflowAssignmentPageTA extends TestBase {
	...
	@FindBy(xpath = "//li/a[contains(text(),'Workflow Assignment')]")
	WebElement wfAssignmentTab;
	@FindBy(id="selected")
	WebElement selectedAgent;
	@FindBy(id="selectAll")
	WebElement selectAllWfCheckbox;
	@FindBy(xpath = "//*[@id='save']/span[2]")
	WebElement saveBtn;
	...
	public void validateSingleWorkflowAssignment(String workflowName) throws Exception {
		...
		//Thread.sleep(4000);
		WebElement wfToAssign=driver.findElement(By.xpath("//*[@title='"+workflowName+"']/input"));
		wfToAssign.click();
		Reporter.log("Single workflow which needs to assign user is selected",true);
		//Thread.sleep(4000);
		...
		//validate count also
		String actual_count=driver.findElement(By.xpath("//span[text()='1']")).getText();
		int actual_wf=Integer.valueOf(actual_count);
		System.out.println("Actual count is:"+actual_wf+"expected count is:1");
	    Assert.assertEquals(actual_wf,1,"No.of wf assigned is not same");
